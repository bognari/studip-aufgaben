 ...
 * @category    Stud.IP
 */
class Leeroy_SimpleORMapCollection extends Leeroy_SimpleCollection
{
    ...
    {
        $ret = new Leeroy_SimpleORMapCollection();
        if (count($data)) {
            $first = current($data);
            if ($first instanceof Leeroy_SimpleORMap) {
     ...
     */
    function __construct(Closure $finder = null, Array $options = null, Leeroy_SimpleORMap $record = null)
    {
     ...
     */
    function setRelatedRecord(Leeroy_SimpleORMap $record)
    {
        $this->related_record = $record;
    }
