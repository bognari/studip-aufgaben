 ...
 * @category    Stud.IP
 */
class Leeroy_SimpleORMap implements ArrayAccess, Countable, IteratorAggregate
{
    /**
     * table row data
     * @var array $content
     ...
     * @return array return values of callback
     */
    public static function findAndMapMany($callable, $pks = array(), $order = '', $params = array())
    {
        $ret = array();
        $calleach = function ($m) use (&$ret, $callable) {
            $ret[] = $callable($m);
    ...
    function __construct($id = null)
    {
        ...
        if (!isset($this->db_fields['id'])
            ...
            if (count($this->pk) === 1) {
                ...
                $this->additional_fields['id'] = array('get' => function ($r, $f) {
                    return is_null($r->getId()) ? null : join('_', $r->getId());
                },
                    'set' => function ($r, $f, $v) {
                        return $r->setId(explode('_', $v));
                    });
            }
        }
    ...
    protected function parseRelationOptions($type, $name, $options)
    {
        ...
        if ($options['foreign_key'] instanceof Closure) {
            $options['assoc_func_params_func'] = function ($record) use ($name, $options) {
                return call_user_func($options['foreign_key'], $record, $name, $options);
            };
        } else {
            $options['assoc_func_params_func'] = function ($record) use ($name, $options) {
                return $options['foreign_key'] === 'id' ? $record->getId() : $record->getValue($options['foreign_key']);
            };
        }
        $that = $this;
        if ($options['assoc_foreign_key'] instanceof Closure) {
            if ($type === 'belongs_to') {
                $options['assoc_foreign_key_getter'] = function ($record) use ($name, $options, $that) {
                    return call_user_func($options['assoc_foreign_key'], $record, $name, $options, $that);
                };
            } else {
                $options['assoc_foreign_key_setter'] = function ($record, $params) use ($name, $options) {
                    return call_user_func($options['assoc_foreign_key'], $record, $params, $name, $options);
                };
            }
        } else {
            if ($type === 'belongs_to') {
                $options['assoc_foreign_key_getter'] = function ($record) use ($name, $options, $that) {
                    return $record->getValue($options['assoc_foreign_key']);
                };
            } else {
                $options['assoc_foreign_key_setter'] = function ($record, $value) use ($name, $options) {
                    return $record->setValue($options['assoc_foreign_key'], $value);
                };
            }
        }
        return $options;
    ...
    public function initRelation($relation)
    {
        ...
        if ($this->relations[$relation] === null) {
            ...
            if ($options['type'] === 'has_many') {
                $records = function ($record) use ($to_call, $params) {
                    $p = (array)$params($record);
                    return call_user_func_array($to_call, count($p) ? $p : array(null));
                };
                $this->relations[$relation] = new Leeroy_SimpleORMapCollection($records, $options, $this);
            } elseif ($options['type'] === 'has_and_belongs_to_many') {
                $records = function ($record) use ($to_call, $params, $options) {
                    $p = (array)$params($record);
                    return call_user_func_array($to_call, array_merge(count($p) ? $p : array(null), array($options)));
                };
                $this->relations[$relation] = new Leeroy_SimpleORMapCollection($records, $options, $this);
            } else {
                $p = (array)$params($this);
